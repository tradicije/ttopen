...
$home_logo = $home_id > 0 ? get_the_post_thumbnail($home_id, 'thumbnail', ['class' => 'stkb-match-logo']) : '';
$away_logo = $away_id > 0 ? get_the_post_thumbnail($away_id, 'thumbnail', ['class' => 'stkb-match-logo']) : '';

get_header();
?>
<main id="primary" class="stkb-match-page" style="max-width:1100px;margin:0 auto;padding:20px 16px;">
    <section class="stkb-match-hero" style="margin-bottom:18px;">
        <div class="stkb-match-meta" style="margin-bottom:10px;opacity:.8;display:flex;gap:10px;flex-wrap:wrap;">
        ...
        <div class="stkb-match-scoreline" style="display:grid;grid-template-columns:1fr auto 1fr;gap:18px;align-items:center;background:#0f172a;color:#fff;border-radius:14px;padding:18px;">
