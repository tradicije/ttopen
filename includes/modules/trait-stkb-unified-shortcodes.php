...
trait STKB_Unified_Shortcodes_Trait
{
    ...
    private static function db_get_top_players_data($liga_slug, $sezona_slug = '', $max_kolo = null)
    {
        ...
        // Optimizacija: jedan agregatni upit za sve setove umesto upita po partiji (N+1).
        $game_ids = [];
        foreach ($rows as $row) {
            $gid = intval($row->id ?? 0);
            if ($gid > 0) {
    ...
    private static function db_get_player_season_club_history($player_id)
    {
        ...
        usort($history, function ($a, $b) {
            $ak = self::season_sort_key((string) ($a['season_slug'] ?? ''));
            $bk = self::season_sort_key((string) ($b['season_slug'] ?? ''));
            if ($ak === $bk) {
    ...
    private static function db_get_match_mvp_player_id($match_id)
    {
        ...
        $games = self::db_get_games_for_match_id($match_id);
        if (empty($games)) {
            return 0;
        }
