...
final class STKB_Unified_Core
{
    ...
    {
        $done = (string) get_option(self::OPTION_REWRITE_FLUSHED, '');
        if ($done === '1') {
            return;
        }
        flush_rewrite_rules(false);
    ...
    {
        $state = (string) get_option(self::OPTION_ONBOARDING_STATE, '');
        if ($state !== '') {
            return;
        }

        if (self::is_existing_install_detected()) {
            update_option(self::OPTION_ONBOARDING_STATE, 'not_needed', false);
            return;
        }
    ...
    {
        $schema_opt = get_option(self::OPTION_SCHEMA_VERSION, null);
        if ($schema_opt !== null) {
            return true;
        }

        $has_any_posts = static function ($post_type) {
        ...
        if ($has_any_posts('klub')) {
            return true;
        }
        if ($has_any_posts('igrac')) {
            return true;
        }
        if ($has_any_posts('pravilo_takmicenja')) {
            return true;
        }
    ...
    {
        if (!is_admin() || wp_doing_ajax() || !current_user_can(self::CAP)) {
            return;
        }
        ...
        if ($page === 'stkb-unified-onboarding') {
            return;
        }
    ...
    {
        $defaults = self::default_visual_settings();
        $in = is_array($raw) ? $raw : [];
        $out = $defaults;
        ...
        $show_titles = isset($in['show_shortcode_titles']) ? (int) $in['show_shortcode_titles'] : 0;
        $out['show_shortcode_titles'] = $show_titles === 1 ? 1 : 0;
        return $out;
    }

    private static function get_visual_settings()
    {
        $saved = get_option(self::OPTION_VISUAL_SETTINGS, []);
        return self::sanitize_visual_settings($saved);
    }
    ...
    {
        $s = self::get_visual_settings();
        return !empty($s['show_shortcode_titles']);
    }

    private static function build_visual_settings_css($settings)
    {
        $s = self::sanitize_visual_settings($settings);
    ...
    {
        // Aktivni model: klub/igrac CPT + taksonomije za ligu i kolo.
...
HTML;
    ...
    {
        if (empty($_FILES[$file_field]) || !isset($_FILES[$file_field]['tmp_name'])) {
    ...
    private static function validate_import_payload($payload, $sections)
    {
        ...
        $sections = self::sanitize_transfer_sections($sections);
        $data = isset($payload['data']) && is_array($payload['data']) ? $payload['data'] : [];
        $summary = self::summarize_import_payload($payload, $sections);
    ...
    public static function handle_save_club_admin()
    {
        ...
        $opstina_opts = self::municipality_options_admin();
    ...
    public static function handle_save_player_admin()
    {
        ...
        if (!array_key_exists($country_code, self::country_options_admin())) {
            $country_code = 'RS';
        }
    ...
    private static function competition_rules_dropdown_admin($name, $selected_id, $required = true)
    {
        ...
        foreach ($rows as $r) {
            $liga_slug = (string) get_post_meta($r->ID, 'stkb_pravila_liga_slug', true);
            $sezona_slug = (string) get_post_meta($r->ID, 'stkb_pravila_sezona_slug', true);
            $label = self::slug_to_title($liga_slug) . ' / ' . self::slug_to_title($sezona_slug);
            if (trim($label) === '/') {
    ...
    private static function players_dropdown_admin($name, $selected, $club_id, $required)
    {
        ...
        // Sačuvaj istorijski unos: ako je već izabran igrač, prikaži ga i kada više nije u tom klubu.
    ...
    private static function get_player_club_id($player_id)
    {
        ...
        $club_id = self::extract_id(get_post_meta($player_id, 'povezani_klub', true));
        if ($club_id > 0) {
            return $club_id;
        }

        $club_id = self::extract_id(get_post_meta($player_id, 'klub_igraca', true));
        if ($club_id > 0) {
            return $club_id;
        }
